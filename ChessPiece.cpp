...
{
    if (board->getPiece(toX, toY) != nullptr && board->getPiece(toX, toY)->getIsWhite() == isWhite)
        return false;
    if (toX == x && toY == y)
        return false;
    return true;
...
bool ChessPiece::canAttack(int x, int y, const ChessBoard* board)
{
    return canMoveTo(x, y, board);
}
